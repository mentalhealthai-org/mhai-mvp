...
LoveAI MVP could always use more documentation, whether as part of the official
LoveAI MVP docs, in docstrings, or even on the web in blog posts, articles, and
such.
...
Ready to contribute? Here’s how to set up `loveai-mvp` for local development.

1.  Fork the `loveai-mvp` repo on GitHub.
1.  Clone your fork locally and change to the directory of your project:

```bash
$ git clone git@github.com:your_name_here/loveai-mvp.git
$ cd loveai-mvp/
```
...
1.  `loveai-mvp` uses a set of `pre-commit` hooks to improve code quality. The
...
The tests can be executed using the `test` dependencies of `loveai-mvp` in the
following way:
...
```bash
$ python -m pytest --cov=loveai-mvp tests/
```
